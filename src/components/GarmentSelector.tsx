  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      >
        ...
        {t("Back to Upload", "Back to Upload")}
      </button>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 md:p-8 shadow-xl border border-slate-100 dark:border-slate-800 relative overflow-hidden">
        ...
        <div className="flex items-center gap-4 mb-4 md:mb-6">
          ...
          <div>
            <h3 className="text-lg md:text-xl font-bold text-slate-800 dark:text-white">{t("Deep Analysis Result", "Deep Analysis Result")}</h3>
            <p className="text-slate-500 dark:text-slate-400 text-xs md:text-sm">{t("AI has analyzed gender, age, and style", "AI has analyzed gender, age, and style")}</p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <label className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">{t("Garment Type", "Garment Type")}</label>
          ...
          {/* Special User Note */}
          <div className="space-y-3">
            <label className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">{t("Special Instructions / Highlights", "Special Instructions / Highlights")}</label>
            <textarea
              value={userNote}
              onChange={(e) => onUserNote(e.target.value)}
              placeholder={t("E.g., Highlight the embroidery on the sleeves, focus on the collar design...", "E.g., Highlight the embroidery on the sleeves, focus on the collar design...")}
          ...
          {recommendation.category !== "full_set" && recommendation.complementaryOptions && (
            <div className="space-y-3 p-6 bg-brand-50 dark:bg-brand-900/20 rounded-2xl border border-brand-100 dark:border-brand-900/40">
              <label className="text-sm font-bold text-brand-700 dark:text-brand-400 uppercase tracking-wider flex items-center gap-2">
                {t("Complete the Look", "Complete the Look")}
                <Sparkles className="w-4 h-4" />
              </label>
              <p className="text-xs text-brand-600 dark:text-brand-400 mb-4">
                {t("This looks like a", "This looks like a")} <strong>{t(recommendation.category, recommendation.category)}</strong>. {t("How should we style the other half?", "How should we style the other half?")}
              </p>
                ...
                {recommendation.complementaryOptions.map((option) => (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-4 md:line-clamp-none">{t(option.description, option.description)}</p>
                    </div>
                  </button>
                ))}
                ...
                <div className={`w-full md:w-[200px] flex-shrink-0 md:snap-center p-4 rounded-xl text-left transition-all border-2 flex flex-col aspect-square md:aspect-auto ${
                  ...
                  {selectedComplementary?.label === "Custom" ? (
                    <textarea
                      ...
                      placeholder={t("E.g., A bright red silk skirt...", "E.g., A bright red silk skirt...")}
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 cursor-pointer" onClick={() => setSelectedComplementary({ label: "Custom", description: "" })}>
                      {t("Write your own brief description for the other half.", "Write your own brief description for the other half.")}
                    </p>
                  )}
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Gender */}
            <div className="space-y-3">
              <label className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">{t("Gender", "Gender")}</label>
            ...
            {/* Age Group */}
            <div className="space-y-3">
              <label className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">{t("Age Group", "Age Group")}</label>
              <div className="flex flex-wrap gap-2">
                {AGE_GROUPS.map((a) => (
                  <button
                    key={a}
          ...
          {/* Style */}
          <div className="space-y-3">
            <label className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">{t("Style Context", "Style Context")}</label>
            <input
              type="text"
              value={style}
              ...
              placeholder={t("e.g. Traditional, Formal, Casual...", "e.g. Traditional, Formal, Casual...")}
            />
          </div>
          ...
          {recommendation.footwearOptions && (
            <div className="space-y-3 p-6 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-slate-700">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">
                {t("AI suggested footwear that best suits the overall garment style.", "AI suggested footwear that best suits the overall garment style.")}
              </p>
        ...
        <div className="mt-8 pt-8 border-t border-slate-100 dark:border-slate-800 flex justify-center sm:justify-end">
          ...
          >
            {t("Confirm & Continue", "Confirm & Continue")}
